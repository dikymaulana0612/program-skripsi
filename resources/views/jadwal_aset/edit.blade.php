                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form class="row g-3 needs-validation myForm" method="POST"
                                    action="{{ route('dashboard.jadwal_aset.update', $data->id) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="col-md-4">
                                        ...
                                        <select name="aset_id" id="aset_id" class="form-control">
                                            ...
                                            @foreach ($asets as $item)
                                                <option value="{{ $item->id }}" {{ $item->id ?? $data->aset_id ? 'selected' : '' }}>{{ $item->nama }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Tgl Jam</label>
                                        <input type="datetime-local" class="form-control" name="tgl_jam" value="{{ $data->tgl_jam }}" required="">
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Max Pengunjung</label>
                                        <input type="number" class="form-control" name="max_pengunjung" value="{{ $data->max_pengunjung }}" required="">
                                    </div>
                                    <!-- end col -->
                                    <div class="col-12">
